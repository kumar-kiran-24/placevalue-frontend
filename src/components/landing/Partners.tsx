    ...
    <div className="mt-14 grid md:grid-cols-3 gap-12 lg:gap-20">
      {tiers.map((t, index) => (
        <motion.div
          ...
          className={`group relative overflow-hidden glass-strong rounded-2xl p-8 transition-all duration-500 cursor-pointer ${
            t.featured ? "shadow-glow-orange" : ""
          }`}
        >
          {/* Glow Effect */}
          ...
          <a
            href="/contact"
            className={`mt-8 inline-flex items-center justify-center w-full py-3 rounded-xl font-medium transition-all duration-500 hover:scale-105 ${
              t.featured
                ? "bg-gradient-to-r from-[var(--brand-blue)] to-[var(--brand-orange)] text-background hover:shadow-lg"
